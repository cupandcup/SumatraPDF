...
inline bool IsContinuous(DisplayMode mode) {
    return DM_CONTINUOUS == mode || DM_CONTINUOUS_FACING == mode || DM_CONTINUOUS_BOOK_VIEW == mode || DM_THUMBNAIL == mode;
}
...
inline bool IsThumbnail(DisplayMode displayMode)
{
    return DM_THUMBNAIL == displayMode;
}
