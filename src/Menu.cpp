...
void MenuUpdateDisplayMode(WindowInfo* win) {
    ...
    for (int id = IDM_VIEW_LAYOUT_FIRST; id <= IDM_VIEW_LAYOUT_LAST; id++) {
        ...
        win::menu::SetEnabled(win->menu, IDM_VIEW_THUMBNAIL, enabled);
    }
    ...
    if (IsSingle(displayMode)) {
        ...
        id = IDM_VIEW_BOOK;
    } else if (IsThumbnail(displayMode)) {
        id = IDM_VIEW_THUMBNAIL;
    }
    else {
    ...
    win::menu::SetChecked(win->menu, IDM_VIEW_THUMBNAIL, IsThumbnail(displayMode));
...
static MenuDef menuDefView[] = {
    ...
    { _TRN("&Thumbnail View\tCtrl+9"),      IDM_VIEW_THUMBNAIL,         MF_NOT_FOR_CHM | MF_NOT_FOR_EBOOK_UI },
    { SEP_ITEM,                             0,                          MF_NOT_FOR_CHM },
